...
import { Search, BookOpen, Clock, ChevronRight, Compass, MapPin, Layers, MessageCircle, Gamepad2, X, Star, Zap, Trophy, Brain, Target, Timer, CheckCircle2, ArrowRight, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto pb-20">
      ...
      <section className="official-card p-8 md:p-10 bg-white dark:bg-slate-900 space-y-8">
        ...
        {/* Game info card — clickable */}
        <motion.button
          whileHover={{ scale: 1.015 }}
          whileTap={{ scale: 0.98 }}
          onClick={() => setShowGameInfo(true)}
          className={cn('w-full flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-primary/5 dark:bg-white/5 border border-primary/10 dark:border-white/10 hover:bg-primary/10 dark:hover:bg-white/10 hover:border-accent/30 transition-all cursor-pointer text-start', dir === 'rtl' ? 'flex-row' : '')}
        >
          <div className="w-9 h-9 rounded-xl bg-primary/10 dark:bg-white/10 flex items-center justify-center shrink-0">
          ...
          <ChevronRight className={cn('w-4 h-4 text-slate-300 dark:text-slate-600 shrink-0', dir === 'rtl' ? 'rotate-180' : '')} />
        </motion.button>
      </section>

      {/* ── Cognitive Game Info Modal ── */}
      <AnimatePresence>
        {showGameInfo && (
          <>
            <motion.div
            ...
            >
              ...
              >
                ...
                <div className="bg-primary rounded-t-[2rem] px-8 pt-8 pb-10 relative overflow-hidden">
                  ...
                  <button
                    onClick={() => setShowGameInfo(false)}
                    className={cn('absolute top-5 w-9 h-9 rounded-xl bg-white/10 border border-white/15 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-all', dir === 'rtl' ? 'left-5' : 'right-5')}
                  >
                    <X className="w-4 h-4" />
                  </button>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-[9px] font-black text-accent/70 uppercase tracking-[0.3em] mb-1">
                        {language === 'ar' ? 'ميزة تفاعلية' : 'Interactive Feature'}
                      </div>
                ...
                <div className="px-8 py-7 space-y-7">
                  ...
                  {/* Description */}
                  <p className={cn('text-sm text-slate-600 dark:text-slate-300 font-semibold leading-relaxed', dir === 'rtl' ? 'text-right' : 'text-left')}>
                    {language === 'ar'
                  ...
                  <div className="space-y-3">
                    <h4 className={cn('text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest', dir === 'rtl' ? 'text-right' : 'text-left')}>
                      {language === 'ar' ? 'كيف تعمل اللعبة؟' : 'How does it work?'}
                    </h4>
                    {[
                      {
                        icon: Brain,
                      ...
                      <div key={i} className={cn('flex items-start gap-3', dir === 'rtl' ? 'flex-row-reverse' : '')}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { icon: Star, label: language === 'ar' ? 'وسام مستكشف' : 'Explorer Badge', xp: '+50 XP', color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800/50' },
                        { icon: Trophy, label: language === 'ar' ? 'وسام باحث' : 'Researcher Badge', xp: '+75 XP', color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800/50' },
                        { icon: Zap, label: language === 'ar' ? 'وسام متميز' : 'Distinguished Badge', xp: '+100 XP', color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800/50' },
                      ].map((r, i) => (
                  ...
                  {/* Tip */}
                  <div className={cn('flex items-start gap-2.5 p-4 rounded-2xl bg-accent/8 dark:bg-accent/10 border border-accent/20', dir === 'rtl' ? 'flex-row-reverse text-right' : 'text-left')}>
                  ...
                  <div className={cn('flex gap-3', dir === 'rtl' ? 'flex-row-reverse' : '')}>
                    <button
                      onClick={() => { setShowGameInfo(false); navigate('/cognitive-ar'); }}
