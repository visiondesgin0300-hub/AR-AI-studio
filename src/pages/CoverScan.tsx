...
interface CoverScanProps {
  // When embedded inside ArHub, the hub owns the top chrome (tab switcher,
  // close button) so CoverScan's own title chip and close button would
  // just duplicate/overlap it.
  embedded?: boolean;
  onMatch?: (book: Book) => void;
  onGoToShelf?: (book: Book) => void;
}

export function CoverScan({ embedded = false, onMatch, onGoToShelf }: CoverScanProps) {
            ...
            <div className={cn('official-card bg-white dark:bg-slate-900 p-6 flex items-center gap-5 shadow-2xl', dir === 'rtl' ? 'flex-row-reverse text-right' : 'flex-row text-left')}>
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <button
                    onClick={() => (onGoToShelf ? onGoToShelf(matchedBook) : navigate('/map', { state: { bookId: matchedBook.id } }))}
