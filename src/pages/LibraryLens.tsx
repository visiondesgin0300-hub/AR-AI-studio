...
export function LibraryLens() {
  ...
  return (
    <div
      className="fixed inset-0 bg-black overflow-hidden select-none cursor-crosshair"
      dir={dir}
      onClick={handleTap}
    >
      {/* Camera */}
              ...
              <div className="bg-black/80 backdrop-blur-2xl rounded-2xl border border-accent/40 overflow-hidden shadow-2xl">
                ...
                <div className="px-3 py-2.5 flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-accent text-[9px] font-black">
                        {ar ? 'رف' : 'Shelf'} {card.book.shelf}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {card.reason ? (
                  <div className="px-3 pb-2">
                    <p className="text-white/35 text-[9px] leading-relaxed line-clamp-2">{card.reason}</p>
                  </div>
                ) : null}
                ...
                <div className="px-3 pb-3">
                  <button
                    onClick={e => {
                      e.stopPropagation();
                      navigate('/map', { state: { bookId: card.book.id } });
                    }}
      ...
      <div className="absolute top-0 left-0 right-0 flex flex-col items-center pt-10 gap-2 pointer-events-none">
        <div className={cn(
          'px-4 py-2 rounded-full backdrop-blur-xl border text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-colors',
          scanning
          ...
          <motion.div
            ...
            transition={{ duration: scanning ? 0.5 : 1.8, repeat: Infinity }}
            className={cn('w-2 h-2 rounded-full', scanning ? 'bg-accent' : 'bg-emerald-400')}
          />
          {scanning
            ? (ar ? 'جاري التحليل...' : 'Analyzing...')
            : (ar ? 'عدسة المكتبة الذكية' : 'Smart Library Lens')}
        </div>
        ...
        <AnimatePresence>
          {scanning && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              className="flex items-center gap-1.5 text-accent text-[9px] font-black uppercase tracking-widest"
            >
